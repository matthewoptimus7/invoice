...
const Invoice = ({
            ...
            if (hasEmptyValue) {
                ...
                if (isSignatureAbsent) {
                    ...
                    try {
                        ...
                        setLoading(true);
                        // add/invoice
                        const response = await axios.post(`${API_URL}invoice/add_invoice/invoice`, 
                            { invoice: inputValuesAboveRows, invoiceitem: previewInvoiceprop, totalValues: totalSum },{
                                headers:{
                        ...
                        if (response.data.status) {
                            ...
                            // save-pdf-server
                            const responseAfterMail = await axios.post(`${API_URL}invoice/after_insert/save-pdf-server`, formData, {
                                headers: {
                                    'Content-Type': 'multipart/form-data',
                                    Authorization: `Bearer ${jwt_token}`
                                },
    ...
    useEffect(() => {
        const statusApiAction = async () => {
            ...
            try {
                // get/signature
                const response = await axios.post(`${API_URL}invoice/get_invoice/signature`, {
    ...
    const generatePDF = async () => {
        if (isSignatureAbsent) {
            ...
            try {
                ...
                setLoading(true);
                // add/ProformaInvoice
                const response = await axios.post(`${API_URL}invoice/add_invoice/ProformaInvoice`,
                    { invoice: inputValuesAboveRows, invoiceitem: previewInvoiceprop, totalValues: totalSum },{
                        headers:{
                            Authorization: `Bearer ${jwt_token}`
                        }
