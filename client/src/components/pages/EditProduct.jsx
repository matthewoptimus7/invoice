...
const EditProduct = () => {
    ...
    useEffect(() => {
        const device_user_data = async () => {
            ...
            try {
                // get/product
                const response = await axios.post(`${API_URL}products/get_product/before_update`, { userid, productid: productid, batchno: productBatch },{
                    headers:{
    ...
    const inputFields = [
        ...
        { label: "Invert Price", name: "priceperitem", value: inputValues.priceperitem, icon: person },
        { label: "CGST", name: "CGST", value: inputValues.CGST, icon: person },
        { label: "SGCT", name: "SGCT", value: inputValues.SGCT, icon: person },
        { label: "MRP" , name: "mrp" , value: inputValues.mrp, icon: person }
    ]
    ...
    const handleClick = async () => {
        ...
        const isValidmrp = /^[0-9]+$/.test(inputValues.mrp);
        if (isValidhsncode & isValidbatchno & isValidproductname & isValidQuantityNo & isValidpriceperitem & isValidCGST & isValidSGCT & isValidmrp) {
            try {
                // console.log("hai", inputValues);
                // update/product
                const response = await axios.put(`${API_URL}products/update_product/update_data`, { productdetial: inputValues, updator: userInfo.userid, batchno: productBatch },{
                    headers:{
                        Authorization: `Bearer ${jwt_token}`
                    }
                });
