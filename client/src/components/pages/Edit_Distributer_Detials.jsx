...
const Edit_Distributer_Detials = ({ Positionid_val }) => {
    ...
    // const jwt_token = sessionStorage.getItem("UserInfo");
    const userInfo = decodeToken(token_jwt).userId;
    // console.log(userid);
    ...
    useEffect(() => {
        const fetchData = async () => {
            console.log("state function calling in view");
            try {
                // /get/state
                const dropDownUserResponse = await axios.post(`${API_URL}user/get/state`,{},{
                    headers:{
                        Authorization: `Bearer ${token_jwt}`
                    }
                });
        ...
        const fetchdistrict = async () => {
            console.log("districti function calling in view");
            try {
                // get/district
                const dropDownUserResponse = await axios.post(`${API_URL}user/get/district`,{},{
                    headers:{
                        Authorization: `Bearer ${token_jwt}`
                    }
                });
        ...
        if (isValiduserid & isValidaadharNo & isValidfName & isValidlName & isValidemail & isValidMobileNo) {
            if (!(Positionid_val === 4 || Positionid_val === 5)) {
                ...
                if (isValidbussinessType & isValidOrgName & isValidgstNumber & isValidpanNumber
                    ...
                    try {
                        // update/user
                        const response = await axios.put(`${API_URL}user/edit/userdata`, { inputValues, AccessOptions: accessValues },{
                            headers:{
                                Authorization: `Bearer ${token_jwt}`
                            }
                        });
                ...
                <div className="new_device_content scroll_div">
                    ...
                    <div className="row_two display-flex padding-loc">
                        ...
                        <Button variant="outlined " style={{
                            color: 'lightblue', borderColor: 'lightblue', width: '71px',
                            height: '71px',
                            borderRadius: '50px'
