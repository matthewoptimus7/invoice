...
const Add_User_Detials = ({ Positionid_val }) => {
    ...
    // const decode_token = decodeToken(token_jwt);
    // console.log("decoded token");
    // console.log(decode_token.userId);
    
    const userInfoString = decodeToken(token_jwt);
        ...
        if (isValiduserid & isValidaadharNo & isValidfName & isValidlName & isValidemail & isValidMobileNo) {
            ...
            if (!(Positionid_val === 4 || Positionid_val === 5)) {
                ...
                if (isValidbussinessType & isValidOrgName & isValidgstNumber & isValidpanNumber
                    ...
                    try {
                        setLoading(true);
                        // add/user
                        const response = await axios.post(`${API_URL}user/add/new_user`,
                            // formData
            ...
            // manufacturer staff and distributor staff insertion condition
                ...
                try {
                    setLoading(true);
                    // add/user
                    const response = await axios.post(`${API_URL}user/add/new_user`, { userDetials: postData, AccessControls: accessValues, Currentuser: userInfo.userid },
                        {
                            headers:{
                                Authorization: `Bearer ${token_jwt}`
                            }
