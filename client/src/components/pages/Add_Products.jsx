...
const Add_Products = () => {
    ...
    const handleClick = async () => {
        ...
        const isValidmrp = /^[0-9]+$/.test(postData.mrp);
        // console.log(isValidhsncode, userInfo.userid);
        // console.log(postData);
        if (isValidhsncode & isValidbatchno & isValidproductname & isValidproductname & isValidQuantityNo & isValidpriceperitem & isValidCGST & isValidSGCT & isValidmrp) {
            try {
                // add/products
                    console.log("before submission")
                    console.log(postData)
                    console.log(userInfo.userid)
                const response = await axios.post(`${API_URL}products/add_product/data`, { productdetial: postData, updator: userInfo.userid },{
                    headers:{
    ...
    const inputFields = [
        ...
        { label: "Invert Price", name: "priceperitem", value: postData.priceperitem, icon: person },
        
        { label: "CGST", name: "CGST", value: postData.CGST, icon: person },
        { label: "SGCT", name: "SGCT", value: postData.SGCT, icon: person },

        { label: "MRP", name: "mrp", value: postData.mrp, icon: person }
    ]
