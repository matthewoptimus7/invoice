...
const ProfilePage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // get/profileInfo
                await axios.post(`${API_URL}user/get_user_data/profileInfo`, {userid:userid},{
                    headers:{
    ...
    const handleSubmit = async (event) => {
        ...
        if (isValiduserid && isValidaadharNo && isValidfName && isValidlName && isValidemail && isValidMobileNo
            ...
            try {
                ...
                setLoading(true);
                // profile_update/user
                const response = await axios.put(`${API_URL}user/profile_udpate/user`, { userId:userId, userDetails: profileInfoRes },{
                    headers:{
                        Authorization: `Bearer ${jwt_token}`
                    }
                });
    ...
    const updateSignApi = async () => {
        ...
        if (isImagePresent) {
            ...
            console.log(file);
            const response = await axios.post(`${API_URL}user/upload_signature/data`,
                formData,
                {
                    headers:{
